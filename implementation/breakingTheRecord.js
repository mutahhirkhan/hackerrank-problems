 ...
 * 
 * @param {Array<number>} scores score made in each game 
 * @returns {Array<number>} first element will be maximum record broken count 2nd will be lowest score count.
 */
function breakingRecords(scores) {
	let maxScoreCount = 0;
	let minScoreCount = 0;

	//setting the default highest and lowest score to first score made
	let max = scores[0],
		min = scores[0];

	scores.forEach((scr) => {
		//if current score is higher than previous
		//increase max record count
		if (scr > max) {
			max = scr;
			maxScoreCount++;
		}

		//otherwise if current score is lowest ever
		//increase lowest score count
		else if (scr < min) {
			minScoreCount++;
			min = scr;
		}
